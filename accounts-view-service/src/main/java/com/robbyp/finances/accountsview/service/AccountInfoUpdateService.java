...
public class AccountInfoUpdateService {
  ...
  public void create(String accountId, String customerId, String title, BigDecimal initialBalance, String description,
                     Int128 version) {
    try {
      AccountChangeInfo ci = new AccountChangeInfo();
      ci.setAmount(MoneyUtil.toIntegerRepr(initialBalance));
      WriteResult x = mongoTemplate.upsert(new Query(where("id").is(accountId).and("version").exists(false)),
                                           new Update()
                                             .set("customerId", customerId)
                                             .set("title", title)
                                             .set("description", description)
                                             .set("balance", MoneyUtil.toIntegerRepr(initialBalance))
                                             .push("changes", ci)
